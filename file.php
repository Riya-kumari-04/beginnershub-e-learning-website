...
if(isset($_POST['submit'])){
    ...
    if(strlen($password) < 8 || !preg_match("/[!@#$%^&*()-_=+]/", $password)){
        ...
        if(mysqli_num_rows($check_result) > 0){
            ...
            // Email is unique and password meets requirements, proceed with insertion
            // Use blowfish encryption for password security
            $hashed_password = password_hash($password, PASSWORD_BCRYPT);
            $insertquery = "INSERT INTO signup (username, email, password) VALUES ('$name', '$email', '$hashed_password')";
